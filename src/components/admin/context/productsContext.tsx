...
import { GET, DELETE } from "../../../api/api_utility";
...
/* ✅ API using your utility */
const fetchProducts = async (
  page: number,
  limit: number,
  filters: Filters
): Promise<ProductsResponse> => {
  const res = await GET<ProductsResponse>("/api/v1/product/getallproducts", {
    params: {
      page,
      limit,
...
export const ProductProvider = ({ children }: { children: ReactNode }) => {
  ...
  /* 🔥 Optimistic delete using your API */
  const deleteProduct = async (product: Product) => {
    ...
    try {
      await DELETE(`/api/v1/product/delete/${product._id}`);
